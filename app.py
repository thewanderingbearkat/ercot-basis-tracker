...
dart_data = {
    "hourly_records_tenaska": [],
    "hourly_records_trailing": [],
    "daily": {},
    "daily_asset": {},
    "monthly": {},
    "total_dart_uplift": 0,
...
def dart_build_price_index(market_prices):
    """Pre-index market prices by (node, date, hour) for O(1) lookups."""
    da_index = {}   # (node, date_str, he) -> price
    rt_index = {}   # (node, date_str, he) -> avg_price
    rt_collect = {} # (node, date_str, hour_start) -> [prices]
    for node in DART_NODES:
        for iso_str, price in market_prices.get(node, {}).get("DASPP", {}).items():
            try:
                dt = datetime.fromisoformat(iso_str)
                he = dt.hour + 1
                if he == 0: he = 24
                da_index[(node, dt.strftime("%Y-%m-%d"), he)] = price
            except:
                continue
        for iso_str, price in market_prices.get(node, {}).get("RTSPP", {}).items():
            try:
                dt = datetime.fromisoformat(iso_str)
                he = dt.hour + 1
                if he == 0: he = 24
                key = (node, dt.strftime("%Y-%m-%d"), he)
                if key not in rt_collect:
                    rt_collect[key] = []
                rt_collect[key].append(price)
            except:
                continue
    for key, prices in rt_collect.items():
        rt_index[key] = sum(prices) / len(prices)
    return da_index, rt_index
...
def calculate_shadow_dart(market_prices, generation, forecasts, forecast_mode="tenaska"):
    ...
    da_index, rt_index = dart_build_price_index(market_prices)
    records = []
    for asset in DART_ASSETS:
        ...
        for hour_key, gen_data in sorted(asset_gen.items()):
            ...
            da_price = da_index.get((node, date_str, he))
            rt_price = rt_index.get((node, date_str, he))
            da_hub = da_index.get(("HB_WEST", date_str, he))
            rt_hub = rt_index.get(("HB_WEST", date_str, he))
...
<body style="background-color: #f8f9fa;">
    ...
    <script>
        ...
        setInterval(fetchPnlData, 2100000);
        setInterval(fetchDartData, 2100000);  // Refresh DART every 35 minutes
        setInterval(updatePharosTimestamp, 1800000);
