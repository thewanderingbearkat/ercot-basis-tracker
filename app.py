...
# ============================================================================
# DART STRATEGY ANALYSIS CONFIGURATION
# ============================================================================
DART_HISTORY_FILE = 'dart_history.json'
...
DART_ASSETS = ["BKI", "BKII", "HOLSTEIN"]  # ERCOT assets only (NWOH already bids DA)
...
# Shadow DART strategy data
dart_data = {
    "hourly_records_tenaska": [],    # Hourly DART records (Tenaska forecast mode)
    "hourly_records_trailing": [],   # Hourly DART records (trailing avg mode)
    "daily": {},                     # Daily aggregates
    "daily_asset": {},               # Daily per-asset aggregates
    "monthly": {},                   # Monthly aggregates
    "total_dart_uplift": 0,
    "total_da_revenue": 0,
    "total_rt_only_revenue": 0,
    "last_dart_update": None,
    "forecast_availability": {},     # Which assets have Tenaska forecasts
}
...
# ============================================================================
# DART STRATEGY - DATA FETCHING & CALCULATIONS
# ============================================================================
def save_dart_data(data):
    """Save DART data to JSON file."""
    try:
        with open(DART_HISTORY_FILE, 'w') as f:
            json.dump(data, f, default=str)
        logger.info(f"Saved DART data to {DART_HISTORY_FILE}")
    except Exception as e:
        logger.error(f"Error saving DART data: {e}")

def load_dart_data():
    """Load DART data from JSON file."""
    try:
        if os.path.exists(DART_HISTORY_FILE):
            with open(DART_HISTORY_FILE, 'r') as f:
                data = json.load(f)
            logger.info(f"Loaded DART data from {DART_HISTORY_FILE}")
            return data
        return None
    except Exception as e:
        logger.error(f"Error loading DART data: {e}")
        return None

def dart_identify_asset(element_name):
    """Identify DART asset from element name (only '- Gen' or '- Generation')."""
...
def fetch_market_prices_da_rt(start_date, end_date, token):
    """Fetch DASPP (hourly) + RTSPP (15-min) for DART settlement points."""
    headers = {"Authorization": f"Bearer {token}"}
    ...
    logger.info(f"DART: Fetching market prices (DASPP+RTSPP) from {start_date} to {end_date}")
    ...
    while current_dt <= end_dt:
        ...
        try:
            ...
            logger.warning(f"DART: Error fetching market prices for {day_str}: {e}")
        current_dt += timedelta(days=1)

    for node in DART_NODES:
        logger.info(f"  DART prices {node}: DASPP={len(result[node]['DASPP'])}, RTSPP={len(result[node]['RTSPP'])}")
    return result

def fetch_dart_generation(start_date, end_date, token):
    """Fetch hourly gen from DART-Energy-Details for BKI/BKII/HOLSTEIN."""
    headers = {"Authorization": f"Bearer {token}"}
    ...
    logger.info(f"DART: Fetching generation from {start_date} to {end_date}")
    ...
    while current_dt <= end_dt:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for item in data.get("data", []):
                    elem = item.get("element", "")
                    asset = dart_identify_asset(elem)
                    if asset == "UNKNOWN":
                        continue
                    hourly_gen = {}
                    hourly_rt_amt = {}
            ...
            logger.warning(f"DART: Error fetching generation for {day_str}: {e}")
        current_dt += timedelta(days=1)

    for asset in DART_ASSETS:
        logger.info(f"  DART gen {asset}: {len(result[asset])} hourly records")
    return result
...
def fetch_dart_forecasts(start_date, end_date, token):
    ...
    logger.info(f"DART: Fetching forecasts from {start_date} to {end_date}")
    ...
    while current_dt <= end_dt:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for item in data.get("data", []):
                    elem = item.get("element", "")
                    asset = DART_FORECAST_ELEMENT_MAP.get(elem)
                    if not asset:
                        asset = dart_identify_asset(elem)
            ...
            logger.warning(f"DART: Error fetching forecasts for {day_str}: {e}")
        current_dt += timedelta(days=1)

    forecast_availability = {}
    for asset in DART_ASSETS:
        nonzero = sum(1 for v in result[asset].values() if v > 0)
        forecast_availability[asset] = nonzero > 0
        logger.info(f"  DART forecast {asset}: {len(result[asset])} intervals, {nonzero} non-zero")
    return result, forecast_availability

def compute_dart_trailing_avg(generation, target_date, lookback_days=DART_TRAILING_AVG_DAYS):
    """Compute trailing N-day hourly average of actual generation as forecast."""
...
def dart_get_hourly_rt_price(market_prices, node, date_str, he):
    """Average 15-min RTSPP intervals to hourly."""
    hour_start = he - 1
    prices = []
...
def dart_get_da_price(market_prices, node, date_str, he):
    """Get DA price for a given hour."""
    hour_start = he - 1
...
def calculate_shadow_dart(market_prices, generation, forecasts, forecast_mode="tenaska"):
    """Core shadow DART calculation - returns hourly records for all DART assets."""
    records = []
    for asset in DART_ASSETS:
        ...
        for hour_key, gen_data in sorted(asset_gen.items()):
            ...
            da_price = dart_get_da_price(market_prices, node, date_str, he)
            rt_price = dart_get_hourly_rt_price(market_prices, node, date_str, he)
            da_hub = dart_get_da_price(market_prices, "HB_WEST", date_str, he)
            rt_hub = dart_get_hourly_rt_price(market_prices, "HB_WEST", date_str, he)
...
def aggregate_dart_results(hourly_records):
    """Aggregate hourly DART records into daily/monthly summaries."""
    daily = defaultdict(lambda: {
        "da_revenue": 0, "rt_deviation_settlement": 0, "shadow_dart_total": 0,
        "rt_only_revenue": 0, "dart_uplift": 0, "gen_mwh": 0, "forecast_mwh": 0, "hours": 0,
    })
    ...
    monthly = defaultdict(lambda: {
        "da_revenue": 0, "rt_deviation_settlement": 0, "shadow_dart_total": 0,
        "rt_only_revenue": 0, "dart_uplift": 0, "gen_mwh": 0, "forecast_mwh": 0, "days": 0,
    })
    ...
    return {
        ...
        "total_dart_uplift": round(sum(d["dart_uplift"] for d in daily.values()), 2),
        "total_da_revenue": round(sum(d["da_revenue"] for d in daily.values()), 2),
        "total_rt_only_revenue": round(sum(d["rt_only_revenue"] for d in daily.values()), 2),
    }

def refresh_dart_data():
    """Fetch all DART data, calculate both forecast modes, aggregate, and save."""
    global dart_data
    try:
        token = get_tenaska_token()
        if not token:
            logger.error("DART: Could not get API token")
            return
        end_date = datetime.now(ZoneInfo("America/Chicago")).strftime("%Y-%m-%d")
        start_date = TENASKA_FETCH_START_DATE
        # Fetch data
        market_prices = fetch_market_prices_da_rt(start_date, end_date, token)
        generation = fetch_dart_generation(start_date, end_date, token)
        api_forecasts, forecast_availability = fetch_dart_forecasts(start_date, end_date, token)
        # Build trailing avg forecasts for each date that has generation data
        all_dates = set()
        for asset in DART_ASSETS:
            for hk in generation.get(asset, {}):
                all_dates.add(hk.split(" ")[0])
        trailing_forecasts = {k: {} for k in DART_ASSETS}
        for d in sorted(all_dates):
            day_trailing = compute_dart_trailing_avg(generation, d)
            for asset in DART_ASSETS:
                trailing_forecasts[asset].update(day_trailing[asset])
        # Calculate both modes
        records_tenaska = calculate_shadow_dart(market_prices, generation, api_forecasts, "tenaska")
        records_trailing = calculate_shadow_dart(market_prices, generation, trailing_forecasts, "trailing_avg")
        agg_tenaska = aggregate_dart_results(records_tenaska)
        agg_trailing = aggregate_dart_results(records_trailing)
        with data_lock:
            dart_data["hourly_records_tenaska"] = records_tenaska
            dart_data["hourly_records_trailing"] = records_trailing
            dart_data["agg_tenaska"] = agg_tenaska
            dart_data["agg_trailing"] = agg_trailing
            dart_data["last_dart_update"] = datetime.now().isoformat()
            dart_data["forecast_availability"] = forecast_availability
        save_dart_data(dart_data)
        logger.info(f"DART: Refresh complete. Tenaska uplift=${agg_tenaska['total_dart_uplift']:,.0f}, Trailing uplift=${agg_trailing['total_dart_uplift']:,.0f}")
    except Exception as e:
        logger.error(f"DART: Error refreshing data: {e}")
        import traceback
        logger.error(traceback.format_exc())
...
def background_data_fetch():
    ...
    try:
        ...
        logger.error(f"Error fetching initial ERCOT data: {e}")
        initial_history = []
    ...
    try:
        ...
        logger.error(f"Error fetching initial PJM data: {e}")
        fresh_pjm_history = []
    ...
    while True:
        try:
            loop_count += 1
            # Log keepalive every ~10 iterations (~20 minutes) so we can detect dead loops
            if loop_count % 10 == 1:
                logger.info(f"Background loop iteration {loop_count} (thread alive)")
            ...
            # Periodic DART strategy refresh (same cadence as Tenaska PnL)
            dart_should_refresh = False
            if last_dart_fetch_time is None:
                dart_should_refresh = True
            else:
                dart_seconds = (datetime.now() - last_dart_fetch_time).total_seconds()
                if dart_seconds >= TENASKA_FETCH_INTERVAL:
                    dart_should_refresh = True
            if dart_should_refresh:
                logger.info("Refreshing DART strategy data...")
                try:
                    refresh_dart_data()
                    last_dart_fetch_time = datetime.now()
                except Exception as e:
                    logger.error(f"Error refreshing DART data: {e}")
...
def load_caches_if_needed():
    ...
    with _cache_lock:
        ...
        try:
            ...
            cached_dart = load_dart_data()
            if cached_dart:
                with data_lock:
                    dart_data.update(cached_dart)
                logger.info(f"Pre-loaded DART cache: uplift=${dart_data.get('agg_trailing', {}).get('total_dart_uplift', 0):,.0f}")
        except Exception as e:
...
def start_background_thread_if_needed():
    ...
    global _background_thread_started, _background_thread
    # Detect and recover from dead background thread
    if _background_thread_started and _background_thread and not _background_thread.is_alive():
        logger.warning("Background thread died! Restarting...")
        _background_thread_started = False
    if not _background_thread_started:
        _background_thread_started = True
        _background_thread = threading.Thread(target=background_data_fetch, daemon=True)
...
# ============================================================================
# DART STRATEGY API ENDPOINTS
# ============================================================================
@app.route('/api/dart/summary', methods=['GET'])
@login_required
def get_dart_summary():
    """Full DART summary. Query param: mode=tenaska|trailing_avg (default: trailing_avg)."""
    mode = request.args.get('mode', 'trailing_avg')
    with data_lock:
        if mode == 'tenaska':
            agg = dart_data.get("agg_tenaska", {})
        else:
            agg = dart_data.get("agg_trailing", {})
        return jsonify({
            "mode": mode,
            "daily": agg.get("daily", {}),
            "daily_asset": agg.get("daily_asset", {}),
            "monthly": agg.get("monthly", {}),
            "total_dart_uplift": agg.get("total_dart_uplift", 0),
            "total_da_revenue": agg.get("total_da_revenue", 0),
            "total_rt_only_revenue": agg.get("total_rt_only_revenue", 0),
            "last_update": dart_data.get("last_dart_update"),
            "forecast_availability": dart_data.get("forecast_availability", {}),
        })

@app.route('/api/dart/hourly/<date>', methods=['GET'])
@login_required
def get_dart_hourly(date):
    """24-hour DART breakdown for a specific date. Query param: mode=tenaska|trailing_avg."""
    mode = request.args.get('mode', 'trailing_avg')
    with data_lock:
        if mode == 'tenaska':
            records = dart_data.get("hourly_records_tenaska", [])
        else:
            records = dart_data.get("hourly_records_trailing", [])
...
@app.route('/api/dart/reload', methods=['POST'])
@login_required
def reload_dart():
    """Force DART data refresh."""
    try:
        refresh_dart_data()
        with data_lock:
            return jsonify({
                "success": True,
                "last_update": dart_data.get("last_dart_update"),
                "total_uplift_tenaska": dart_data.get("agg_tenaska", {}).get("total_dart_uplift", 0),
                "total_uplift_trailing": dart_data.get("agg_trailing", {}).get("total_dart_uplift", 0),
            })
    except Exception as e:
        logger.error(f"Error reloading DART data: {e}")
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/dart/status', methods=['GET'])
@login_required
def get_dart_status():
    """DART data status summary."""
    with data_lock:
        return jsonify({
            "last_update": dart_data.get("last_dart_update"),
            "forecast_availability": dart_data.get("forecast_availability", {}),
            "tenaska_records": len(dart_data.get("hourly_records_tenaska", [])),
            "trailing_records": len(dart_data.get("hourly_records_trailing", [])),
            "tenaska_uplift": dart_data.get("agg_tenaska", {}).get("total_dart_uplift", 0),
            "trailing_uplift": dart_data.get("agg_trailing", {}).get("total_dart_uplift", 0),
        })
...
<body style="background-color: #f8f9fa;">
    <div class="p-3 md:p-4" style="background: linear-gradient(135deg, #f8f9fa 0%, #ffffff 100%);">
        <div class="max-w-7xl mx-auto">
            ...
            <!-- ============================================================ -->
            <!-- DART STRATEGY ANALYSIS SECTION -->
            <!-- ============================================================ -->
            <div class="mb-2 mt-6">
                <div class="flex justify-between items-center">
                    <h2 class="text-lg font-bold" style="color: var(--skyvest-navy); border-left: 3px solid #f59e0b; padding-left: 8px;">Shadow DART Strategy Analysis</h2>
                    <div class="flex items-center gap-2">
                        <div class="flex rounded overflow-hidden border" style="border-color: var(--skyvest-navy);">
                            <button id="dart-mode-trailing" onclick="setDartMode('trailing_avg')" class="px-3 py-1 text-xs font-semibold" style="background-color: var(--skyvest-navy); color: white;">Trailing 7-Day Avg</button>
                            <button id="dart-mode-tenaska" onclick="setDartMode('tenaska')" class="px-3 py-1 text-xs font-semibold" style="background-color: white; color: var(--skyvest-navy);">Tenaska Forecast</button>
                        </div>
                        <button onclick="reloadDartData()" class="px-3 py-1 text-xs font-semibold rounded" style="background-color: var(--skyvest-gold); color: var(--skyvest-navy);">Reload</button>
                    </div>
                </div>
                <p class="text-xs mt-1" style="color: #999;">Theoretical DA+RT settlement vs current RT-only for BKI, McCrae (BKII), Holstein</p>
            </div>

            <!-- DART Summary Cards -->
            <div class="grid grid-cols-1 md:grid-cols-4 gap-2 mb-3">
                <div class="card rounded-sm p-3" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);">
                    <p class="metric-label mb-1" style="color: rgba(255,255,255,0.8);">DART Uplift</p>
                    <span id="dart-total-uplift" class="text-2xl font-bold text-white">$0</span>
                    <p class="text-xs mt-1" style="color: rgba(255,255,255,0.7);">DA Revenue + RT Dev vs RT-Only</p>
                </div>
                <div class="card rounded-sm p-3">
                    <p class="metric-label mb-1" style="color: #666;">Shadow DART Total</p>
                    <span id="dart-shadow-total" class="text-2xl font-light" style="color: var(--skyvest-navy);">$0</span>
                </div>
                <div class="card rounded-sm p-3">
                    <p class="metric-label mb-1" style="color: #666;">RT-Only (Current)</p>
                    <span id="dart-rt-only" class="text-2xl font-light" style="color: var(--skyvest-navy);">$0</span>
                </div>
                <div class="card rounded-sm p-3">
                    <p class="metric-label mb-1" style="color: #666;">DA Revenue Component</p>
                    <span id="dart-da-revenue" class="text-2xl font-light" style="color: var(--skyvest-navy);">$0</span>
                </div>
            </div>
            ...
            <div class="card rounded-sm p-3 mb-3">
                <div class="flex items-center gap-2 mb-2">
                    <span class="text-xs font-semibold" style="color: #666;">Period:</span>
                    <div class="flex rounded overflow-hidden border" style="border-color: var(--skyvest-navy);">
                        <button id="dart-period-daily" onclick="setDartPeriod('daily')" class="px-3 py-1 text-xs font-semibold dart-period-btn" style="background-color: white; color: var(--skyvest-navy);">Daily</button>
                        <button id="dart-period-mtd" onclick="setDartPeriod('mtd')" class="px-3 py-1 text-xs font-semibold dart-period-btn" style="background-color: white; color: var(--skyvest-navy);">MTD</button>
                        <button id="dart-period-ytd" onclick="setDartPeriod('ytd')" class="px-3 py-1 text-xs font-semibold dart-period-btn active" style="background-color: var(--skyvest-navy); color: white;">YTD</button>
                    </div>
                </div>
            </div>

            <!-- DART Asset Comparison Table -->
            <div class="card rounded-sm p-3 mb-3">
                <h3 class="text-sm font-semibold mb-2" style="color: var(--skyvest-navy); border-bottom: 1px solid #e5e5e5; padding-bottom: 4px;">Per-Asset DART Comparison</h3>
                <div style="overflow-x: auto;">
                    <table class="w-full text-sm">
                        <thead>
                            <tr style="border-bottom: 2px solid var(--skyvest-navy);">
                                <th class="text-left py-2 px-2" style="color: var(--skyvest-navy);">Asset</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">DA Revenue</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">RT Dev Sett</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">DART Total</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">RT-Only</th>
                                <th class="text-right py-2 px-2" style="color: #f59e0b;">Uplift</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">Gen MWh</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">Fcst MWh</th>
                            </tr>
                        </thead>
                        <tbody id="dart-asset-table-body">
                            <tr><td colspan="8" class="text-center py-4" style="color: #999;">Loading DART data...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>

            <!-- DART Daily Detail Table -->
            <div class="card rounded-sm p-3 mb-3">
                <div class="flex justify-between items-center mb-2">
                    <h3 class="text-sm font-semibold" style="color: var(--skyvest-navy);">DART by Period</h3>
                    <span id="dart-forecast-note" class="text-xs" style="color: #999;"></span>
                </div>
                <div id="dart-detail-container" style="max-height: 400px; overflow-y: auto;">
                    <table class="w-full text-sm">
                        <thead>
                            <tr style="border-bottom: 2px solid var(--skyvest-navy);">
                                <th class="text-left py-2 px-2" style="color: var(--skyvest-navy);">Date</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">DA Revenue</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">RT Dev Sett</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">DART Total</th>
                                <th class="text-right py-2 px-2" style="color: var(--skyvest-navy);">RT-Only</th>
                        ...
                        <tbody id="dart-detail-table-body">
                            <tr><td colspan="7" class="text-center py-4" style="color: #999;">Loading...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>

            <!-- DART Hourly Strip Chart -->
            <div class="card rounded-sm p-3 mb-3">
                <div class="flex justify-between items-center mb-2">
                    <h3 class="text-sm font-semibold" style="color: var(--skyvest-navy);">Today's Hourly DART Performance</h3>
                    <span id="dart-hourly-date" class="text-xs px-2 py-0.5 rounded" style="background-color: #f59e0b; color: white;">--</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        <p class="text-xs font-semibold mb-1" style="color: var(--skyvest-navy);">BKI (Bearkat I)</p>
                        <div id="dart-hourly-bki" style="display: flex; gap: 1px; height: 60px; align-items: flex-end; background: #f1f5f9; border-radius: 4px; padding: 2px;"></div>
                    </div>
                    <div>
                        <p class="text-xs font-semibold mb-1" style="color: var(--skyvest-navy);">BKII (McCrae)</p>
                        <div id="dart-hourly-bkii" style="display: flex; gap: 1px; height: 60px; align-items: flex-end; background: #f1f5f9; border-radius: 4px; padding: 2px;"></div>
                    </div>
                    <div>
                        <p class="text-xs font-semibold mb-1" style="color: var(--skyvest-navy);">Holstein</p>
                        <div id="dart-hourly-holstein" style="display: flex; gap: 1px; height: 60px; align-items: flex-end; background: #f1f5f9; border-radius: 4px; padding: 2px;"></div>
                    </div>
                </div>
                <div class="flex gap-4 mt-2 text-xs" style="color: #666;">
                    <span><span style="display:inline-block;width:10px;height:10px;background:#22c55e;border-radius:2px;margin-right:4px;"></span>Forecast</span>
                    <span><span style="display:inline-block;width:10px;height:10px;background:var(--skyvest-blue);border-radius:2px;margin-right:4px;"></span>Actual Gen</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // ============================================================
        // DART STRATEGY ANALYSIS - JavaScript
        // ============================================================
        const DART_API_URL = window.location.protocol + '//' + window.location.host + '/api/dart';
        let dartData = null;
        let dartMode = 'trailing_avg';
        let dartPeriod = 'ytd';

        function setDartMode(mode) {
            dartMode = mode;
            document.getElementById('dart-mode-trailing').style.backgroundColor = mode === 'trailing_avg' ? 'var(--skyvest-navy)' : 'white';
            document.getElementById('dart-mode-trailing').style.color = mode === 'trailing_avg' ? 'white' : 'var(--skyvest-navy)';
            document.getElementById('dart-mode-tenaska').style.backgroundColor = mode === 'tenaska' ? 'var(--skyvest-navy)' : 'white';
            document.getElementById('dart-mode-tenaska').style.color = mode === 'tenaska' ? 'white' : 'var(--skyvest-navy)';
            fetchDartData();
        }

        function setDartPeriod(period) {
            dartPeriod = period;
            document.querySelectorAll('.dart-period-btn').forEach(btn => {
                btn.style.backgroundColor = 'white';
                btn.style.color = 'var(--skyvest-navy)';
            });
            const activeBtn = document.getElementById('dart-period-' + period);
            if (activeBtn) {
                activeBtn.style.backgroundColor = 'var(--skyvest-navy)';
        ...
        async function fetchDartData() {
            try {
                const resp = await fetch(DART_API_URL + '/summary?mode=' + dartMode);
                dartData = await resp.json();
                updateDartDisplay();
                fetchDartHourly();
            } catch (e) {
                console.error('Error fetching DART data:', e);
            }
        }

        function fmtDart(val) {
        ...
        function updateDartDisplay() {
            if (!dartData) return;
            const daily = dartData.daily || {};
            const monthly = dartData.monthly || {};
            const dailyAsset = dartData.daily_asset || {};
            const now = new Date();
            ...
            let filteredDays = {};
            let filteredAssetDays = {};
            if (dartPeriod === 'daily') {
                if (daily[todayStr]) filteredDays[todayStr] = daily[todayStr];
                if (dailyAsset[todayStr]) filteredAssetDays[todayStr] = dailyAsset[todayStr];
            } else if (dartPeriod === 'mtd') {
                Object.keys(daily).filter(d => d.startsWith(monthStr)).forEach(d => { filteredDays[d] = daily[d]; });
                Object.keys(dailyAsset).filter(d => d.startsWith(monthStr)).forEach(d => { filteredAssetDays[d] = dailyAsset[d]; });
            } else {
                filteredDays = daily;
                filteredAssetDays = dailyAsset;
            }

            // Compute totals from filtered days
            let totalUplift = 0, totalDart = 0, totalRtOnly = 0, totalDaRev = 0;
            Object.values(filteredDays).forEach(d => {
                totalUplift += d.dart_uplift || 0;
                totalDart += d.shadow_dart_total || 0;
                totalRtOnly += d.rt_only_revenue || 0;
                totalDaRev += d.da_revenue || 0;
            });

            // Update summary cards
            const upliftEl = document.getElementById('dart-total-uplift');
            upliftEl.textContent = fmtDart(totalUplift);
            upliftEl.style.color = totalUplift >= 0 ? 'white' : '#fecaca';
            document.getElementById('dart-shadow-total').textContent = fmtDart(totalDart);
            document.getElementById('dart-rt-only').textContent = fmtDart(totalRtOnly);
            document.getElementById('dart-da-revenue').textContent = fmtDart(totalDaRev);

            // Forecast note
            const avail = dartData.forecast_availability || {};
            const noFcst = Object.entries(avail).filter(([k,v]) => !v).map(([k]) => k);
            document.getElementById('dart-forecast-note').textContent = dartMode === 'tenaska' && noFcst.length > 0
                ? 'Note: No Tenaska forecast for ' + noFcst.join(', ') + ' (using 0 DA bid)'
                : 'Mode: ' + (dartMode === 'trailing_avg' ? 'Trailing 7-Day Avg' : 'Tenaska Forecast');

            // Per-asset breakdown (aggregate across filtered days)
            const assetTotals = {};
            Object.values(filteredAssetDays).forEach(dayAssets => {
                Object.entries(dayAssets).forEach(([asset, d]) => {
                    if (!assetTotals[asset]) assetTotals[asset] = {da_revenue:0,rt_deviation_settlement:0,shadow_dart_total:0,rt_only_revenue:0,dart_uplift:0,gen_mwh:0,forecast_mwh:0};
                    assetTotals[asset].da_revenue += d.da_revenue || 0;
                    assetTotals[asset].rt_deviation_settlement += d.rt_deviation_settlement || 0;
                    assetTotals[asset].shadow_dart_total += d.shadow_dart_total || 0;
                    assetTotals[asset].rt_only_revenue += d.rt_only_revenue || 0;
                    assetTotals[asset].dart_uplift += d.dart_uplift || 0;
                    assetTotals[asset].gen_mwh += d.gen_mwh || 0;
                    assetTotals[asset].forecast_mwh += d.forecast_mwh || 0;
                });
            });

            const assetNames = {BKI: 'Bearkat I', BKII: 'McCrae (BKII)', HOLSTEIN: 'Holstein'};
            const assetBody = document.getElementById('dart-asset-table-body');
            if (Object.keys(assetTotals).length === 0) {
                assetBody.innerHTML = '<tr><td colspan="8" class="text-center py-4" style="color:#999;">No DART data available</td></tr>';
            } else {
                let html = '';
                let grandTotal = {da_revenue:0,rt_deviation_settlement:0,shadow_dart_total:0,rt_only_revenue:0,dart_uplift:0,gen_mwh:0,forecast_mwh:0};
                ['BKI','BKII','HOLSTEIN'].forEach(asset => {
                    const d = assetTotals[asset];
                    if (!d) return;
                    Object.keys(grandTotal).forEach(k => grandTotal[k] += d[k]);
                    const upliftColor = d.dart_uplift >= 0 ? '#22c55e' : '#ef4444';
                    html += '<tr style="border-bottom:1px solid #e5e5e5;">' +
                        '<td class="py-2 px-2 font-semibold" style="color:var(--skyvest-navy);">' + (assetNames[asset]||asset) + '</td>' +
                        '<td class="text-right py-2 px-2">' + fmtDart(d.da_revenue) + '</td>' +
                        '<td class="text-right py-2 px-2">' + fmtDart(d.rt_deviation_settlement) + '</td>' +
                        '<td class="text-right py-2 px-2 font-semibold">' + fmtDart(d.shadow_dart_total) + '</td>' +
                        '<td class="text-right py-2 px-2">' + fmtDart(d.rt_only_revenue) + '</td>' +
                        '<td class="text-right py-2 px-2 font-bold" style="color:' + upliftColor + ';">' + fmtDart(d.dart_uplift) + '</td>' +
                        '<td class="text-right py-2 px-2">' + d.gen_mwh.toLocaleString('en-US',{maximumFractionDigits:1}) + '</td>' +
                        '<td class="text-right py-2 px-2">' + d.forecast_mwh.toLocaleString('en-US',{maximumFractionDigits:1}) + '</td>' +
                        '</tr>';
                });
                // Grand total row
                const gtColor = grandTotal.dart_uplift >= 0 ? '#22c55e' : '#ef4444';
                html += '<tr style="border-top:2px solid var(--skyvest-navy);font-weight:bold;">' +
                    '<td class="py-2 px-2" style="color:var(--skyvest-navy);">Total</td>' +
                    '<td class="text-right py-2 px-2">' + fmtDart(grandTotal.da_revenue) + '</td>' +
                    '<td class="text-right py-2 px-2">' + fmtDart(grandTotal.rt_deviation_settlement) + '</td>' +
                    '<td class="text-right py-2 px-2">' + fmtDart(grandTotal.shadow_dart_total) + '</td>' +
                    '<td class="text-right py-2 px-2">' + fmtDart(grandTotal.rt_only_revenue) + '</td>' +
                    '<td class="text-right py-2 px-2" style="color:' + gtColor + ';">' + fmtDart(grandTotal.dart_uplift) + '</td>' +
                    '<td class="text-right py-2 px-2">' + grandTotal.gen_mwh.toLocaleString('en-US',{maximumFractionDigits:1}) + '</td>' +
                    '<td class="text-right py-2 px-2">' + grandTotal.forecast_mwh.toLocaleString('en-US',{maximumFractionDigits:1}) + '</td>' +
                    '</tr>';
                assetBody.innerHTML = html;
            }

            // Detail table (daily or monthly rows)
            const detailBody = document.getElementById('dart-detail-table-body');
            let detailData = dartPeriod === 'mtd' || dartPeriod === 'ytd' ? filteredDays : filteredDays;
            const sortedDates = Object.keys(detailData).sort().reverse();
            if (sortedDates.length === 0) {
                detailBody.innerHTML = '<tr><td colspan="7" class="text-center py-4" style="color:#999;">No data</td></tr>';
            } else {
                let dHtml = '';
                sortedDates.forEach(date => {
                    const d = detailData[date];
                    const uplColor = d.dart_uplift >= 0 ? '#22c55e' : '#ef4444';
                    dHtml += '<tr style="border-bottom:1px solid #f0f0f0;cursor:pointer;" onclick="fetchDartHourly(\'' + date + '\')">' +
                        '<td class="py-1 px-2" style="color:var(--skyvest-navy);">' + date + '</td>' +
                        '<td class="text-right py-1 px-2">' + fmtDart(d.da_revenue) + '</td>' +
                        '<td class="text-right py-1 px-2">' + fmtDart(d.rt_deviation_settlement) + '</td>' +
                        '<td class="text-right py-1 px-2 font-semibold">' + fmtDart(d.shadow_dart_total) + '</td>' +
                        '<td class="text-right py-1 px-2">' + fmtDart(d.rt_only_revenue) + '</td>' +
                        '<td class="text-right py-1 px-2 font-bold" style="color:' + uplColor + ';">' + fmtDart(d.dart_uplift) + '</td>' +
                        '<td class="text-right py-1 px-2">' + (d.gen_mwh||0).toLocaleString('en-US',{maximumFractionDigits:1}) + '</td>' +
                        '</tr>';
                });
        ...
        async function fetchDartHourly(date) {
            if (!date) {
                const now = new Date();
                date = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0') + '-' + String(now.getDate()).padStart(2,'0');
            }
            document.getElementById('dart-hourly-date').textContent = date;
            try {
                const resp = await fetch(DART_API_URL + '/hourly/' + date + '?mode=' + dartMode);
                const data = await resp.json();
                ['BKI','BKII','HOLSTEIN'].forEach(asset => {
                    const containerId = 'dart-hourly-' + asset.toLowerCase();
                    const container = document.getElementById(containerId);
                    if (!container) return;
                    const records = (data.by_asset || {})[asset] || [];
                    if (records.length === 0) {
                        container.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#999;font-size:11px;">No data</div>';
                        return;
                    }
                    // Find max for scaling
                    let maxVal = 0;
                    records.forEach(r => { maxVal = Math.max(maxVal, Math.abs(r.actual_gen_mw || 0), Math.abs(r.forecast_mw || 0)); });
                    if (maxVal === 0) maxVal = 1;
                    let barsHtml = '';
                    for (let he = 1; he <= 24; he++) {
                        const r = records.find(x => x.hour_ending === he);
                        const fcst = r ? Math.abs(r.forecast_mw) : 0;
                        const actual = r ? Math.abs(r.actual_gen_mw) : 0;
                        const fcstPct = (fcst / maxVal * 100).toFixed(1);
                        const actPct = (actual / maxVal * 100).toFixed(1);
                        barsHtml += '<div style="flex:1;display:flex;flex-direction:column;justify-content:flex-end;gap:0;min-width:0;" title="HE' + he + ': Fcst=' + fcst.toFixed(1) + ' Act=' + actual.toFixed(1) + '">' +
                            '<div style="height:' + fcstPct + '%;background:#22c55e;border-radius:1px 1px 0 0;min-height:' + (fcst > 0 ? '1px' : '0') + ';"></div>' +
                            '<div style="height:' + actPct + '%;background:var(--skyvest-blue);border-radius:0 0 1px 1px;min-height:' + (actual > 0 ? '1px' : '0') + ';"></div>' +
                            '</div>';
                    }
                    container.innerHTML = barsHtml;
                });
            } catch (e) {
                console.error('Error fetching DART hourly:', e);
            }
        }

        async function reloadDartData() {
            try {
                const resp = await fetch(DART_API_URL + '/reload', {method: 'POST'});
                const result = await resp.json();
                if (result.success) {
                    fetchDartData();
                } else {
                    console.error('DART reload failed:', result.message);
                }
            } catch (e) {
                console.error('Error reloading DART data:', e);
            }
        }
        ...
        setInterval(fetchDartData, 2100000);  // Refresh DART every 35 minutes
