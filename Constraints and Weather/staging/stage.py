...
P_NODES = "51301,34497127,1318144721,1369011076,1369012529"
YEARS = 3
...
INSERTS = [
    ...
    ("CM_PJM_DART_DAILY", f"""
        ...
        SELECT OBJECTID, DATETIME::DATE, SUM(RTLMP), SUM(COALESCE(RTCONG,0)), SUM(COALESCE(RTLOSS,0)), COUNT(*)
        FROM {YES}.DART_PRICES
        WHERE OBJECTID IN ({P_NODES}) AND RTLMP IS NOT NULL AND {WIN}
          AND DATETIME::DATE NOT IN (SELECT DISTINCT DAY FROM {DBO}.CM_PJM_DART_DAILY)
        GROUP BY OBJECTID, DATETIME::DATE"""),
