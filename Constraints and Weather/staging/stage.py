...
# ERCOT operating day on FIXED CST (Etc/GMT+6, no DST). The data is America/Chicago
# (observes DST), so this is the Central-market analog of PJM's fixed-EST slice and
# keeps the day boundary stable year-round (no 23/25-hour transition days).
E_DAY = "CONVERT_TIMEZONE('America/Chicago','Etc/GMT+6', DATETIME)::DATE"
...
INSERTS = [
    ("CM_ERCOT_SF_DAILY", f"""
        ...
        SELECT PRICENODEID, {E_DAY}, CONSTRAINTID, ANY_VALUE(CONSTRAINTNAME),
        ...
        WHERE MARKET='RT' AND PRICENODEID IN ({E_NODES}) AND {WIN}
          AND {E_DAY} NOT IN (SELECT DISTINCT DAY FROM {DBO}.CM_ERCOT_SF_DAILY)
        GROUP BY PRICENODEID, {E_DAY}, CONSTRAINTID"""),
    ("CM_ERCOT_INTERVALS_DAILY", f"""
        INSERT INTO {DBO}.CM_ERCOT_INTERVALS_DAILY
        SELECT {E_DAY}, COUNT(DISTINCT DATETIME)
        FROM {YES}.MARKET_SHIFT_FACTORS
        WHERE MARKET='RT' AND PRICENODEID IN ({E_NODES}) AND {WIN}
          AND {E_DAY} NOT IN (SELECT DAY FROM {DBO}.CM_ERCOT_INTERVALS_DAILY)
        GROUP BY {E_DAY}"""),
    ("CM_ERCOT_LMP_DAILY", f"""
        INSERT INTO {DBO}.CM_ERCOT_LMP_DAILY
        SELECT OBJECTID, {E_DAY}, SUM(RTLMP), COUNT(*)
        FROM {YES}.DART_PRICES
        WHERE OBJECTID IN ({E_NODES}) AND RTLMP IS NOT NULL AND {WIN}
          AND {E_DAY} NOT IN (SELECT DISTINCT DAY FROM {DBO}.CM_ERCOT_LMP_DAILY)
        GROUP BY OBJECTID, {E_DAY}"""),
